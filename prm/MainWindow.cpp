...
int WINAPI WinMain(_In_ HINSTANCE hInst, _In_opt_ HINSTANCE hPrevInst, _In_ LPSTR args, _In_ int ncmdshow) {
	...
	CreateWindow(L"MainWndClass", L"PRM", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 100, 100, 800, 600, NULL, NULL, NULL, NULL);
...
LRESULT CALLBACK SoftwareMainProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND:
		switch (wp) {
		case OnMenuSave:
			if (q_init.x == INT_MAX || q_init.y == INT_MAX ||
				q_goal.x == INT_MAX || q_goal.y == INT_MAX) {
				MessageBoxA(hWnd, "Недостаточно данных. Повторите снова", "Ошибка", MB_OK);
			}
			else {
				SaveData();
			}
		...
		case OnButtonStartClicked:
			if (q_init.x == INT_MAX || q_init.y == INT_MAX ||
				...
				points = g.points;
				if (result.empty()) {
					MessageBoxA(hWnd, "Решение не найдено", "Результат", MB_OK);
				}
			}
			InvalidateRect(hWnd, &OBS, TRUE);
			break;
		...
		case OnButtonSetPointsPointClicked:
			...
			if (q_init.x < 0 || q_init.x > 570 || q_init.y < 0 || q_init.y > 470 ||
				...
				MessageBoxA(hWnd, "Некорректный ввод стартовой / финишной позиции робота", "Ошибка", MB_OK);
				startPositionX = INT_MAX;
				startPositionY = INT_MAX;
				finishPositionX = INT_MAX;
				finishPositionY = INT_MAX;
		...
		case OnButtonDeleteCircleClicked:
			if (obstacles.empty()) {
				MessageBoxA(hWnd, "Все препятствия удалены", "Ошибка", MB_OK);
			}
			else {
				g.clear();
				points.clear();
	...
	case WM_PAINT:
		...
		if (!points.empty()) {
			for (int i = 0; i < points.size(); ++i) {
				...
				Ellipse(ps.hdc, 10 + points[i].x - 1, -points[i].y - 1 + 480, 10 + points[i].x + 1, -points[i].y + 1 + 480);
				DeleteObject(hp);
			}
		}
		...
		if (!obstacles.empty()) {
			for (int i = 0; i < obstacles.size(); ++i) {
				if (obstacles[i].c.x != INT_MAX && obstacles[i].c.y != INT_MAX && obstacles[i].r != INT_MAX) {
					hp = CreatePen(PS_ALTERNATE, 1, RGB(255, 184, 65));
					SelectObject(ps.hdc, hp);
					SelectObject(ps.hdc, CreateSolidBrush(RGB(255, 184, 65)));
					Ellipse(ps.hdc, 10 + obstacles[i].c.x - obstacles[i].r,
						-obstacles[i].c.y - obstacles[i].r + 480,
						10 + obstacles[i].c.x + obstacles[i].r,
						-obstacles[i].c.y + obstacles[i].r + 480);
					DeleteObject(hp);
				}
			}
		...
		if (!points.empty()) {
			for (int i = 0; i < points.size() - 1; ++i) {
				...
				for (int j = 0; j < adjacent_vertices.size(); ++j) {
					DrawLine(ps.hdc, 10 + points[i].x, -points[i].y + 480, 10 + points[adjacent_vertices[j]].x, -points[adjacent_vertices[j]].y + 480);
					DeleteObject(hp);
				}
				DeleteObject(hp);
			}
		...
		if (!(startPositionX == INT_MAX || startPositionY == INT_MAX
			...
			Ellipse(ps.hdc, 10 + q_init.x - 3, -q_init.y - 3 + 480, 10 + q_init.x + 3, -q_init.y + 3 + 480);
			Ellipse(ps.hdc, 10 + q_goal.x - 3, -q_goal.y - 3 + 480, 10 + q_goal.x + 3, -q_goal.y + 3 + 480);
			DeleteObject(hp);
		}
		...
		if (!result.empty()) {
			for (int i = 0; i < result.size() - 1; ++i) {
				...
				SelectObject(ps.hdc, hp);
				SelectObject(ps.hdc, CreateSolidBrush(RGB(255, 0, 0)));
				DrawLine(ps.hdc, 10 + result[i].x, -result[i].y + 480, 10 + result[i + 1].x, -result[i + 1].y + 480);
				DeleteObject(hp);
			}
			...
			for (int i = 0; i < result.size(); ++i) {
				...
				if (result[i] != q_init && result[i] != q_goal) {
					Ellipse(ps.hdc, 10 + result[i].x - 2, -result[i].y - 2 + 480, 10 + result[i].x + 2, -result[i].y + 2 + 480);
				}
				DeleteObject(hp);
			}
		}
...
void MainWndAddMenus(HWND hWnd) {
	...
	AppendMenu(SubMenu, MF_STRING, OnMenuSave, L"Сохранить кофигурацию");
	AppendMenu(SubMenu, MF_STRING, OnMenuRead, L"Прочитать конфигурацию");
...
void MainWndAddWidgets(HWND hWnd) {
	CreateWindowA("button", "Задать позиции", WS_VISIBLE | WS_CHILD, 610, 10, 160, 30, hWnd, (HMENU)OnButtonSetPointsPointClicked, NULL, NULL);
	CreateWindowA("button", "Задать препятствие", WS_VISIBLE | WS_CHILD, 610, 50, 160, 30, hWnd, (HMENU)OnButtonSetCircleClicked, NULL, NULL);
	CreateWindowA("button", "Удалить препятствие", WS_VISIBLE | WS_CHILD, 610, 90, 160, 30, hWnd, (HMENU)OnButtonDeleteCircleClicked, NULL, NULL);
	CreateWindowA("button", "Старт", WS_VISIBLE | WS_CHILD, 640, 490, 120, 30, hWnd, (HMENU)OnButtonStartClicked, NULL, NULL);
	CreateWindowA("button", "Очистить", WS_VISIBLE | WS_CHILD, 500, 490, 120, 30, hWnd, (HMENU)OnButtonCleanClicked, NULL, NULL);
}
